...
import NodeGenarator from "./NodeGenrator";
type Node = NumberNode | SumNode;
type Conn =
  | Connection<NumberNode, SumNode>
  | Connection<SumNode, SumNode>
  | Connection<SumNode, NumberNode>;
...
export async function createEditor(container: HTMLElement) {
  ...
  dockNodes.forEach(async (e) => {
    await editor.addNode(new NodeGenarator(process, e as any));
  });
  editor.use(dataflow);
  ...
  dockNodes.forEach((e) => {
    dock.add(() => new NodeGenarator(process, e as any));
  });
  ...
  async function process() {
    ...
    editor
      .getNodes()
      .filter((node) => node instanceof SumNode)
      .forEach(async (node) => {
